#CreamT
#Connect to the Teltonika RMS api to access device information
import requests, json

#setup personal access token in RMS account page
#each token is valid for one year then will need to be renewed 
#see api dev reference for description of REST requests
#https://developers.rms.teltonika-networks.com/pages/api.html

#import device ID from parent module
if __name__ == "__main__":
    deviceID = 273746 #default val
else:
    pass


def api_request(deviceID, query):
    access_token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9..N5DKt6cBBgb7dqXA5vNv1oQ8e0QwoP_rshv5nN0a2f9nZtI6q0EsozL6aIdWHyEpvEuTM4Fyb6pR3AgMrbRQpRCc-VyDJyY4ynXrO1bqbTd0IWLLbDHMl5PnvNm0WPkuKJ8WUK_4TS1PFeI9p6BVNAqrR0NlCxRtzxWgG_IC1Fa5p3SiOXv5Dgqgyea5j03yEGHZFz_olzlNt4gwhEIW4Cif1uQ71zw5drv86NSazv9WwTJ6I1igDHD54VZcqEYhn9d4-ZqUjh9I8YV68vZWAkCvxuY9nYpJuKFlb8-du5PWe4JtlfQEN7uiCOOq0E5jof4PSM2KgGEmbzevHmGkiw'
    headers = {"accept":"application/json", "Authorization":"Bearer " + access_token}
    api_request_url = 'https://rms.teltonika-networks.com/api/devices/'+str(deviceID)+query
    try:
        r = requests.get(api_request_url, headers=headers)
        r.raise_for_status()
        return r
    except requests.exceptions.RequestException as err:
        print ("Oops: Request Error",err)
    except requests.exceptions.HTTPError as errh:
        print ("Http Error:",errh)
    except requests.exceptions.ConnectionError as errc:
        print ("Connection Error:",errc)
    except requests.exceptions.Timeout as errt:
        print ("Timeout Error:",errt)   
    
def get_dev_status(deviceID):
    query = '?fields=status'
    r = api_request(deviceID, query)
    if r:
        data = json.loads(r.text)
        val = data["data"]["status"]
        print("Rut status: \t[{}]" .format(val))
        print(deviceID)
    else:
        val = 0
    return val

def get_mobile_ip(deviceID):
    query = '?fields=mobile_ip'
    r = api_request(deviceID, query)
    if r:
        data = json.loads(r.text)
        val = data["data"]["mobile_ip"]
       # print("Mobile IP: [{}]" .format(val))
    else:
        val = 0
    return val


#print(deviceID)
#get_mobile_ip(deviceID)

"""
deviceStatus = get_dev_status(deviceID)
ipAddr = get_mobile_ip(deviceID)
print(deviceStatus)
print(ipAddr)
"""
"""
if r:
    print(r.text)
    print(r.status_code)

    data = json.loads(r.text)
    ipAddr = data["data"]["mobile_ip"]
    deviceStatus = data["data"]["status"]

    print("Recieved JSON payload:")
    print(json.dumps(data, indent=2))
    print(ipAddr)
    print(deviceStatus)
"""