...
#CreamT libraries
import deviceInfo, rmsApiReq, manageConnection, manageData_Testing, googleAPI
...
while(1):
    ...
    for s in dev:
        ...
        if isinstance(s.tideNext, str):
            s.diff = deviceInfo.time_diff(deviceInfo.to_seconds(s.tideNext))
            #adding 3600 to make a positive number
            s.diff  = s.diff + 3600
            for s in dev:
                print(s.diff)

        if s.diff > 200 and s.scanDl and dlflag:
            print(s.location + ': Checking scans...')
            [locNames, locSizes] = manageData_Testing.list_files_local(filepath)
            [devNames, devSizes] = manageData_Testing.list_files_device(deviceInfo.http_get(url + '?FILE'))
            dlFiles = manageData_Testing.list_new_scans(devNames, devSizes, locNames, locSizes, s.location)
            if len(dlFiles):
                for i in dlFiles:
                    a=0
                    manageData_Testing.get_scan_data(url, s.location, i, filepath)
            [locNames, locSizes] = manageData_Testing.list_files_local(filepath)
            manageData_Testing.split_file(filepath, s.location, locNames, locSizes)
            s.scanDl = False
